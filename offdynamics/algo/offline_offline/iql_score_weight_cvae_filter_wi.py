...
# filter out the samples with the lowest score inside the training loop, not entirely outside at the begining
class IQL_score_weight_cvae_filter_inside(object):
    ...
    def __init__(self,
        ...
        """
        weight_type: the type of weight used in the loss function for src samples
            - max_min_normalize_score: exp(-max_min_normalize_score)
            - weighted_shifted_log_sas_ratio: exp(-shifted_log_sas_ratio) shifted means that we shift the log_sas_ratio by the minimum value
    ...
    def update_q_functions(self, state_batch, action_batch, reward_batch, nextstate_batch, not_done_batch, score_weight, writer=None):
        with torch.no_grad():
        ...
        loss = (score_weight * (q_1 - value_target)**2).mean() + (score_weight * (q_2 - value_target)**2).mean() # weighted loss
        return loss

    def update_policy(self, advantage_batch, state_batch, action_batch, writer):
        ...
        if self.config['policy_regu']:
            pi, _, _ = self.policy(state_batch)
            log_beta = self.vae_policy.dataset_prob(state_batch, pi, beta=0.4, num_samples=10)
            regu_loss = self.config['reg_weight'] * log_beta.mean()
            policy_loss -= regu_loss

        if writer is not None and self.total_it % 5000 == 0:
            writer.add_scalar('train/bc_loss', bc_loss.mean().item(), self.total_it)
            writer.add_scalar('train/policy_loss', policy_loss.item(), self.total_it)
            if self.config['policy_regu']:
                writer.add_scalar('train/regu_loss', regu_loss.item(), self.total_it)
        return policy_loss
    ...
    def train(self, src_replay_buffer, tar_replay_buffer, batch_size=128, writer=None):
        ...
        src_state, src_action, src_next_state, src_reward, src_not_done, src_score_weight = src_replay_buffer.sample(batch_size, score_weight_type=self.weight_type)
        tar_state, tar_action, tar_next_state, tar_reward, tar_not_done = tar_replay_buffer.sample(batch_size)

        keep_batch_size = int(batch_size * (1 - self.config['cut_off']/100))
        filter_score, idxes = torch.topk(src_score_weight.squeeze(), keep_batch_size)
        

        src_state = src_state[idxes]
        src_action = src_action[idxes]
        src_next_state = src_next_state[idxes]
        src_reward = src_reward[idxes]
        src_not_done = src_not_done[idxes]
        src_score_weight = src_score_weight[idxes]
        ...
        tar_score_weight = torch.ones_like(tar_reward) # same batch size with target samples
        score_weight = torch.cat([src_score_weight, tar_score_weight], 0)
        ...
        q_loss_step = self.update_q_functions(state, action, reward, next_state, not_done, score_weight, writer)
        ...
        for p in self.q_funcs.parameters():
            p.requires_grad = False
        pi_loss_step = self.update_policy(adv, state, action, writer)
        ...
        if self.total_it % 5000 == 0:
            writer.add_scalar('train/src_score_weight', src_score_weight.mean(), self.total_it)
            writer.add_scalar('train/keep_batch_size', keep_batch_size, self.total_it)
