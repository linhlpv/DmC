...
def call_mujoco_env(env_config: Dict) -> gym.Env:
    ...
    print(f"env_name: {env_name}, shift_level: {shift_level}")

    # assert the shift level legal
    if 'morph' in env_name or 'kinematic' in env_name:
        assert shift_level in ['easy', 'medium', 'hard', 'ot'], 'The required shift is not available yet, please consider modify the xml file on your own or use the shift scale among easy, medium, hard'
